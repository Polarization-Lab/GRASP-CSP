...
def getdata(f,num_step,data):
    ...
    with f as f:
        ...
        for group in [group1, group2, group3, group4, group5, group6, group7]:
            ...
            for dataset_name in dataset_names:
                if dataset_name in group:
                    if dataset_name in data[data_key]:
                        # If the dataset_name is not already a key in data[data_key][dataset_name], create a new entry with an empty dictionary
                        #data[data_key][dataset_name] = {}
                        # Add a new key-value pair to data[data_key][dataset_name], with the key being the string representation of num_step and the value being the image data
                        data[data_key][dataset_name].update(str(num_step):{ r.image_crop(group[dataset_name][()])})
                    else:
                        data[data_key][dataset_name] = str(num_step): { r.image_crop(group[dataset_name][()])}
